    ...
    if len(first_step.frames) > 0:
        ...
        for fo_key in frame.fieldOutputs.keys():
            ...
            inv_names = ['mises', 'tresca', 'press', 'inv3',
                         ...
                         'outOfPlanePrincipal',
                         'magnitude']  # vector fields (U, V, A, RF, FLVEL, ...)
            if len(fo.values) > 0:
                fv0 = fo.values[0]
...
# Averaging threshold used by Abaqus Visualization (default = 75%%).
# At each node, contributions from adjacent elements are averaged if
# the relative nodal variation is <= threshold; otherwise each
# element's extrapolated value contributes separately to the plot max/min.
AVERAGING_THRESHOLD_PCT = 75.0
...
# Add MAGNITUDE only if supported by this Abaqus version
try:
    INVARIANT_MAP['Magnitude'] = MAGNITUDE
except NameError:
    pass
...
def extract_value(fv, comp_or_inv, field_output):
    ...
    if is_invariant(comp_or_inv):
        ...
        val = getattr(fv, attr_name, None)
        if val is not None:
            return val
        # Fallback for 'Magnitude' on vector fields whose FieldValue
        # doesn't expose .magnitude directly: compute sqrt(sum(data_i^2))
        if comp_or_inv == 'Magnitude':
            try:
                d = fv.data
                if d is None:
                    return None
                # d may be a scalar, tuple, or array-like
                try:
                    iter(d)
                    comps = [float(x) for x in d]
                except TypeError:
                    return float(d)
                s = 0.0
                for x in comps:
...
for elset_name in ELEMENT_SET_NAMES:
    ...
    # -----------------------------------------------------------------
    # PRE-ASSIGN elements to regions (bbox-overlap method)
    # An element is assigned to a region if its bounding box overlaps
    # the region box.  This avoids missing boundary elements whose
    # centroids sit just outside the region - the most common source
    # of incorrect max/min values with centroid-only assignment.
    # Boundary elements that span two adjacent regions are assigned
    # to BOTH, matching Abaqus Visualization query behaviour.
    # -----------------------------------------------------------------
    print('  Assigning elements to %d regions (bbox-overlap, Viewer-equivalent)...' % total_regions)
    ...
    EPS = 1.0e-9   # floating-point tolerance

    # Manual bounding-box overlap.  ODB Python 2.7 does NOT expose
    # getByBoundingBox on OdbMeshElementArray, so we implement the
    # same selection criterion (element bbox intersects region bbox)
    # using node coordinates.  This matches Abaqus/Viewer's
    # "Display Group -> Elements -> By bounding box" selection.
    for elabel in elset_element_labels:
        if elabel not in elem_node_map:
            continue
        ...
        ex_max_y = float('-inf')
        for nLabel in nodes:
            if nLabel not in node_coord_map:
                continue
            nc = node_coord_map[nLabel]
            nx, ny = nc[0], nc[1]
            if nx < ex_min_x: ex_min_x = nx
            if nx > ex_max_x: ex_max_x = nx
            if ny < ex_min_y: ex_min_y = ny
            if ny > ex_max_y: ex_max_y = ny

        if ex_min_x == float('inf'):
            continue

        for ri, region_info in enumerate(REGIONS):
            r_xmin = region_info['x_min']
            r_xmax = region_info['x_max']
            r_ymin = region_info['y_min']
            r_ymax = region_info['y_max']
            if (ex_max_x >= r_xmin - EPS and ex_min_x <= r_xmax + EPS and
                ex_max_y >= r_ymin - EPS and ex_min_y <= r_ymax + EPS):
    ...
    for ri in range(total_regions):
        if len(region_elem_map[ri]) > 0:
            rr = REGIONS[ri]
            print('    Region %d [%.6f..%.6f x %.6f..%.6f]: %d elements, %d nodes'
                  % (ri, rr['x_min'], rr['x_max'], rr['y_min'], rr['y_max'],
                     len(region_elem_map[ri]), len(region_node_map[ri])))
    sys.stdout.flush()
    ...
    # Build element-to-regions lookup
    elem_to_regions = {{}}
    for ri in range(total_regions):
        for el in region_elem_map[ri]:
            elem_to_regions.setdefault(el, []).append(ri)

    # Build PRECISE node-to-regions lookup based on the node's actual
    # coordinate (not merely "node belongs to an element in the region").
    # This is essential for ELEMENT_NODAL extrapolated values: each value
    # at node N is an extrapolation from ONE element, and the value only
    # belongs to a region when N itself sits inside that region.  Using
    # the looser element-based node list would let a neighbouring
    # element's extrapolation bleed into this region via a shared node.
    node_to_regions_precise = {{}}
    for elabel in elset_element_labels:
        if elabel not in elem_node_map:
            continue
        for nLabel in elem_node_map[elabel]:
            if nLabel in node_to_regions_precise:
            ...
            regs = []
            for ri, region_info in enumerate(REGIONS):
                if (region_info['x_min'] - EPS <= nx <= region_info['x_max'] + EPS and
                    region_info['y_min'] - EPS <= ny <= region_info['y_max'] + EPS):
                    regs.append(ri)
            node_to_regions_precise[nLabel] = regs

    # Legacy node_to_regions (for pure nodal field outputs only - e.g. POR)
    node_to_regions = node_to_regions_precise
        ...
        for frame in frames_to_process:
            ...
            for (field_key, comp_or_inv, label) in OUTPUTS_TO_EXTRACT:
              try:
                ...
                # Determine the position type of the raw field output
                ...
                # If 'Magnitude' was requested but this field's FieldValue
                # does not expose it (e.g. scalar/tensor fields), skip so we
                # don't trip a low-level Abaqus error.  The user only selects
                # 'Magnitude' on vector fields via the UI, but be defensive.
                if comp_or_inv == 'Magnitude' and len(field_output.values) > 0:
                    fv0 = field_output.values[0]
                    has_mag = False
                    try:
                        m = getattr(fv0, 'magnitude', None)
                        if m is not None:
                            has_mag = True
                    except Exception:
                        has_mag = False
                    if not has_mag:
                        # try vector data fallback check
                        d = getattr(fv0, 'data', None)
                        is_vec = False
                        try:
                            iter(d); is_vec = True
                        except Exception:
                            is_vec = False
                        if not is_vec:
                            print('    [skip] %s %s: magnitude not available on this field' % (field_key, label))
                            sys.stdout.flush()
                            continue

                # For element-based fields (stress, strain, etc.) try to
                # get ELEMENT_NODAL values (extrapolated from integration
                # points to nodes).  These match what Abaqus Visualization
                # shows in contour plots and probes.
                use_values = field_output.values
                is_element_nodal = False
                if not is_nodal_field:
                    try:
                        fo_enodal = field_output.getSubset(position=ELEMENT_NODAL)
                        if len(fo_enodal.values) > 0:
                            use_values = fo_enodal.values
                            is_element_nodal = True
                    except Exception:
                        pass   # keep original integration-point values

                # -------------------------------------------------------
                # Pass 1: collect per-region contributions, keyed by node
                # for ELEMENT_NODAL (so we can apply Abaqus 75%% averaging).
                # For nodal and integration-point fields we just collect a
                # flat list of values per region.
                # -------------------------------------------------------
                # region_node_contribs[ri][nl] -> list of extrapolated values
                region_node_contribs = {{}}
                # region_flat_values[ri] -> flat list (for nodal/IP fields)
                region_flat_values = {{}}

                for fv in use_values:
                    el = getattr(fv, 'elementLabel', None)
                    nl = getattr(fv, 'nodeLabel', None)

                    if is_element_nodal:
                        # Abaqus display-group-by-bounding-box semantics:
                        # once element E's bbox overlaps region R, the ENTIRE
                        # element is included in the display group.  All of
                        # E's ELEMENT_NODAL values (at every node of E,
                        # including nodes whose coordinates are outside the
                        # region's bbox) contribute to R's legend max/min.
                        if el is None or nl is None:
                            continue
                        ...
                        if nl is None:
                            continue
                        matched_regions = node_to_regions_precise.get(nl, [])
                        if not matched_regions:
                            continue
                    else:
                        if el is None or el not in elem_to_regions:
                            continue
                        matched_regions = elem_to_regions[el]
                    ...
                    if is_element_nodal:
                        for ri in matched_regions:
                            if ri not in region_node_contribs:
                                region_node_contribs[ri] = {{}}
                            bucket = region_node_contribs[ri]
                            if nl not in bucket:
                                bucket[nl] = []
                            bucket[nl].append(val)
                    else:
                        for ri in matched_regions:
                            if ri not in region_flat_values:
                                region_flat_values[ri] = []
                            region_flat_values[ri].append(val)

                # -------------------------------------------------------
                # Pass 2: reduce to per-region max/min.  For ELEMENT_NODAL
                # apply Abaqus Visualization averaging at each node:
                #   variation = (v_max - v_min) / max_abs_in_region * 100
                #   variation <= threshold -> use averaged value
                #   variation >  threshold -> treat contributions as unaveraged
                #                             (each contributes to plot max/min)
                # -------------------------------------------------------
                if is_element_nodal:
                    for ri, nodes_dict in region_node_contribs.items():
                        key = (ri, step_name, frame_idx)
                        if key not in results:
                            continue
                        # Compute max |v| across all contributions in this region
                        max_abs_R = 0.0
                        for nl, vlist in nodes_dict.items():
                            for v in vlist:
                                av = v if v >= 0 else -v
                                if av > max_abs_R:
                                    max_abs_R = av
                        ...
                        for nl, vlist in nodes_dict.items():
                            ...
                            vmin = min(vlist)
                            if max_abs_R > 0:
                                variation = (vmax - vmin) / max_abs_R * 100.0
                            else:
                                variation = 0.0
                            if variation <= AVERAGING_THRESHOLD_PCT:
                                # Averaged value at this node
                                vavg = sum(vlist) / float(len(vlist))
                                effective_vals = [vavg]
                            else:
                                # Variation too high -> unaveraged; each
                                # contribution appears in the plot separately.
                                effective_vals = vlist
                            for v in effective_vals:
                    ...
                    for ri, vlist in region_flat_values.items():
                        key = (ri, step_name, frame_idx)
                        if key not in results:
                            continue
                        cur_max, cur_min = results[key][label]
                ...
                print('    [ERROR] field=%s label=%s in step=%s frame=%s:' % (field_key, label, step_name, frame_idx))
                _tb.print_exc()
                sys.stdout.flush()
                continue
...
def _post_process_to_excel(output_dir):
    ...
    for elset_name in element_sets:
        ...
        # --------------------------------------------------------------
        # Phase assignment: odd-position steps (1st, 3rd, 5th...) = Loading,
        # even-position steps (2nd, 4th, 6th...) = Relaxation.
        # --------------------------------------------------------------
        def _phase_of(step_idx):
            return "Loading" if (step_idx % 2 == 0) else "Relaxation"

        loading_steps = [(i, s) for i, s in enumerate(step_names) if _phase_of(i) == "Loading"]
        relax_steps   = [(i, s) for i, s in enumerate(step_names) if _phase_of(i) == "Relaxation"]

        # --------------------------------------------------------------
        # Helper: write a single step block (header + Y\X grid) starting
        # at `start_row` of worksheet `ws`. Returns next free row index.
        # --------------------------------------------------------------
        def _safe_float(s):
            try:
                return float(s)
        ...
        def _write_step_block(ws, start_row, step, step_idx, label, stat_type):
            phase = _phase_of(step_idx)
            stat_idx = 0 if stat_type == "Max" else 1

            # Header banner row (merged across all X cols + the Y label col)
            banner = "%s  |  %s %s  |  %s" % (step, label, stat_type, phase)
            ws.cell(row=start_row, column=1, value=banner)
            _style_header_row(ws, start_row, len(x_labels) + 1)
            ws.merge_cells(
                start_row=start_row, start_column=1,
                end_row=start_row,   end_column=len(x_labels) + 1
            )

            # Axis header row: "Y \ X" + x labels
            ws.cell(row=start_row + 1, column=1, value="Y \\ X")
            for ci, xl in enumerate(x_labels):
                ws.cell(row=start_row + 1, column=ci + 2, value=xl)
            _style_header_row(ws, start_row + 1, len(x_labels) + 1)

            # Data rows: one per Y label
            for yi_idx, yl in enumerate(y_labels):
                r = start_row + 2 + yi_idx
                cell = ws.cell(row=r, column=1, value=yl)
                cell.font = header_font_white
                cell.fill = header_fill
                cell.border = thin_border
                cell.alignment = Alignment(horizontal="center")
                ...
                for xi_idx in range(len(x_labels)):
                    ...
                    if key in data and label in data[key]:
                        val_str = data[key][label][stat_idx]
                        _write_value(ws, r, xi_idx + 2, val_str)

            # Block occupies: 1 banner + 1 axis + len(y_labels) data + 1 blank
            return start_row + 2 + len(y_labels) + 1

        def _build_stacked_sheet(sheet_name, steps_subset, label, stat_type):
            ws = wb.create_sheet(title=sheet_name)
            cursor = 1
            for step_idx, step in steps_subset:
                cursor = _write_step_block(ws, cursor, step, step_idx, label, stat_type)
            _auto_fit(ws)
            return ws

        # --------------------------------------------------------------
        # Per-variable sheets: Max/Min x Loading/Relax/AllSteps
        # --------------------------------------------------------------
        all_steps = list(enumerate(step_names))
        for label in header_labels:
            for stat_type in ["Max", "Min"]:
                base = "%s_%s" % (label, stat_type)

                def _fit(name):
                    # Excel sheet name limit is 31 chars; truncate label if needed.
                    if len(name) <= 31:
                        return name
                    excess = len(name) - 31
                    return "%s_%s_%s" % (label[:max(1, len(label) - excess)], stat_type, name.split("_")[-1])

                _build_stacked_sheet(_fit("%s_Loading"  % base), loading_steps, label, stat_type)
                _build_stacked_sheet(_fit("%s_Relax"    % base), relax_steps,   label, stat_type)
                _build_stacked_sheet(_fit("%s_AllSteps" % base), all_steps,     label, stat_type)

        # --------------------------------------------------------------
        # Summary sheet (built last, inserted at front)
        # --------------------------------------------------------------
        total_regions = len(x_labels) * len(y_labels)

        ws_sum = wb.create_sheet(title="Summary", index=0)
        ws_sum.cell(row=1, column=1, value="Element Set: %s" % set_short).font = Font(bold=True, size=11)
        ws_sum.cell(row=2, column=1, value="Steps: %d (%d Loading + %d Relaxation)" %
                    (len(step_names), len(loading_steps), len(relax_steps))).font = Font(bold=True, size=11)
        ws_sum.cell(row=3, column=1, value="Regions: %d" % total_regions).font = Font(bold=True, size=11)

        sum_header_row = 5
        ws_sum.cell(row=sum_header_row, column=1, value="Step")
        ws_sum.cell(row=sum_header_row, column=2, value="Phase")
        col = 3
        stat_col_map = {}  # (label, stat) -> col
        for label in header_labels:
            for stat_type in ["Max", "Min"]:
                ws_sum.cell(row=sum_header_row, column=col, value="%s %s" % (label, stat_type))
                stat_col_map[(label, stat_type)] = col
                col += 1
        _style_header_row(ws_sum, sum_header_row, col - 1)

        for si, step in enumerate(step_names):
            r = sum_header_row + 1 + si
            phase = _phase_of(si)
            c1 = ws_sum.cell(row=r, column=1, value=step)
            c1.border = thin_border
            c1.font = header_font
            c2 = ws_sum.cell(row=r, column=2, value=phase)
            c2.border = thin_border

            # For each (label, stat), aggregate over all regions of this step.
            for label in header_labels:
                step_max = None
                step_min = None
                for xi_idx in range(len(x_labels)):
                    for yi_idx in range(len(y_labels)):
                        key = (step, xi_idx, yi_idx)
                        if key not in data or label not in data[key]:
                            continue
                        vmax = _safe_float(data[key][label][0])
                        vmin = _safe_float(data[key][label][1])
                        if vmax is not None:
                            step_max = vmax if step_max is None else max(step_max, vmax)
                        if vmin is not None:
                            step_min = vmin if step_min is None else min(step_min, vmin)
                if step_max is not None:
                    _write_value(ws_sum, r, stat_col_map[(label, "Max")], step_max)
                if step_min is not None:
                    _write_value(ws_sum, r, stat_col_map[(label, "Min")], step_min)
        _auto_fit(ws_sum)
